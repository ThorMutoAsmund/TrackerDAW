...
namespace TrackerDAW
{
    ...
    {
        public static void CheckDragDropMove(this Control conrol, Vector diff, object data, string dataObjectFormat)
        {
            if (Math.Abs(diff.X) > SystemParameters.MinimumHorizontalDragDistance || Math.Abs(diff.Y) > SystemParameters.MinimumVerticalDragDistance)
            {
                // Initialize the drag & drop operation
                DataObject dragData = new DataObject(dataObjectFormat, data);
                DragDrop.DoDragDrop(conrol, dragData, DragDropEffects.Copy);
            }
        }
